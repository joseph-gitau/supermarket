...
<div id="body">
    ...
    <?php if(isset($_GET['list'])){
		...
		if(!strcmp(strtolower($_GET['list']),"product")){
			...
			$plist = mysqli_query($con,"select product_id, product_name, supplier_id, market_price, cost_price from product");
			while($row = mysqli_fetch_array($plist)){
				echo"<tr><td>{$row['product_id']}</td>
					 <td>{$row['product_name']}</td>";
			$slist = mysqli_query($con,"select sdealer from supplier where sid='{$row['supplier_id']}'");
	...
	elseif(!strcmp(strtolower($_GET['list']),"supplier")){
			...
			$slist = mysqli_query($con,"select sid, sname, sdealer, semail, sphone from supplier");
	...
	elseif(!strcmp(strtolower($_GET['list']),"customer")){
			...
			$slist = mysqli_query($con,"select cid, first_name,last_name, cjoin_date, cmoney_spent, caddress,cmoney_spent_reset,cphone from customer");
			while($row = mysqli_fetch_array($slist)){
	...
	elseif(!strcmp(strtolower($_GET['list']),"employee")){
			...
			$slist = mysqli_query($con, "select id, first_name, last_name, salary, admin, dob, phone_number, address, uid, join_date, end_date from employee");
			while($row = mysqli_fetch_array($slist)){
	...
	elseif(!strcmp(strtolower($_GET['list']),"promo")){
			...
			$slist = mysqli_query($con, "select promo_code, discount, valid_upto, count from promotion");
			while($row = mysqli_fetch_array($slist)){
		...
		elseif(!strcmp(strtolower($_GET['list']),"dept")){
			...
			$dlist = mysqli_query($con,"select dept_id, dept_name, manager_id from department");
			while($row = mysqli_fetch_array($dlist)){
				echo"<tr><td>{$row['dept_id']}</td>
					 <td>{$row['dept_name']}</td>";
				if($row['manager_id']){
					$elist = mysqli_query($con, "select first_name, last_name from employee where id='{$row['manager_id']}'");
