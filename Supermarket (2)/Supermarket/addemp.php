...
<div id="body">
	...
	<?php
	   if(isset($_GET['third'])&&isset($_POST['user'])){
		   $user_result=mysqli_query($con, "INSERT INTO login VALUES('{$_POST['user']}',md5('{$_POST['password']}'),id,{$_POST['admin']})");
		   if(!$user_result){
	   ...
	   if(isset($_GET['second'])&&isset($_POST['fname'])){
	   		...
	   		$result = mysqli_query($con, "INSERT INTO employee VALUES('{$_POST['fname']}','{$_POST['lname']}',id,'{$_POST['dept_id']}',{$_POST['salary']},{$_POST['pnum']},'{$_POST['address']}',{$_POST['uid']},'{$_POST['jdate']}','{$_POST['bdate']}','{$_POST['edate']}',{$_POST['perks']},{$_POST['admin']})"); 
	   //page 2 form
	   $empidset = mysqli_query($con, "SELECT id FROM employee where uid='{$_POST['uid']}'");
        	...
        	<table>
                ...
                <tr><td style='padding:5px'>Last Name:</td>
					...
					<tr><td style='padding:5px'>Dept: </td>
						...
						$dept_set = mysqli_query($con,"select dept_id, dept_name from department where manager_id='0'");
