...
<div id="body">
    ...
    <div class="rcontent">
	  ...
	  if(!strcmp(strtolower($_GET['name']),"product") & isset($_GET['success'])){
				$result=mysqli_query($con,"update product set product_name='{$_POST['product_name']}',cost_price={$_POST['cprice']},supplier_id={$_POST['supplier']},quantity={$_POST['quantity']},product_type='{$_POST['product_type']}',market_price={$_POST['mprice']} where product_id='{$_POST['product_id']}'");
				if(!$result)echo "Deletion not successful ".mysqli_error();
	   			else echo"<h1><span>Deletion of product data successful</span></h1>";
			}
			else{
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"product")){
			...
			echo"<div id='data'>";
				...
				$plist=mysqli_query($con,"select * from product where product_id='{$_GET['id']}'");
			...
			echo "<form method='post' action='editlist.php?name=product&success=1'>
					  <table>
						...
						$dept_set = mysqli_query($con,"select dept_id, dept_name from department");
				while($row = mysqli_fetch_array($dept_set))
					...
					echo"</select>
						...
						$supplier_set = mysqli_query($con,"select sid, sname from supplier");
	  ...
	  //supplier start
	  	  if(!strcmp(strtolower($_GET['name']),"supplier") & isset($_GET['success'])){
				$result=mysqli_query($con,"update supplier set sname='{$_POST['name']}',saddress='{$_POST['address']}',sdealer='{$_POST['dealer']}',sphone={$_POST['phone']},semail='{$_POST['email']}' where sid='{$_POST['sid']}'");
				if(!$result)echo "Deletion not successful ".mysql_error();
	   			else echo"<h1><span>Deletion of supplier data successful</span></h1>";
			}
			else{
	   if(isset($_GET['name'])&isset($_GET['id'])){
		//supplier
		if(!strcmp(strtolower($_GET['name']),"supplier")){
			...
			echo"<div id='data'>";
				...
				$plist=mysqli_query($con, "select * from supplier where sid='{$_GET['id']}'");
				$plist=mysqli_fetch_array($plist);
	  ...
	  //customer start
	  	  if(!strcmp(strtolower($_GET['name']),"customer") & isset($_GET['success'])){
				$result=mysqli_query($con,"update customer set first_name='{$_POST['fname']}',last_name='{$_POST['lname']}',caddress='{$_POST['caddress']}',cphone={$_POST['cphone']} where cid='{$_POST['cid']}'");
				if(!$result)echo "Deletion not successful ".mysqli_error();
	   			else echo"<h1><span>Deletion of customer data successful</span></h1>";
			}
			else{
	   if(isset($_GET['name'])&isset($_GET['id'])){
		//customer
		if(!strcmp(strtolower($_GET['name']),"customer")){
			...
			echo"<div id='data'>";
				...
				$plist=mysqli_query($con, "select * from customer where cid='{$_GET['id']}'");
				$plist=mysqli_fetch_array($plist);
